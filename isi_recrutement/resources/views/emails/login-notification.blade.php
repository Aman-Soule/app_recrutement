<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="utf-8">
    <title>Nouvelle connexion</title>
</head>
<body style="font-family: Arial, sans-serif; background:#f4f5f7; padding:24px; color:#1f2937;">
    <table role="presentation" style="max-width:480px; margin:0 auto; background:#ffffff; border-radius:8px; overflow:hidden;">
        <tr>
            <td style="background:#2563eb; padding:20px 24px;">
                <span style="color:#ffffff; font-size:18px; font-weight:bold;">TalentAI</span>
            </td>
        </tr>
        <tr>
            <td style="padding:24px;">
                <h2 style="margin-top:0;">Bonjour {{ $user->name }},</h2>
                <p>Une connexion vient d'avoir lieu sur votre compte TalentAI.</p>
                <table role="presentation" style="width:100%; border-collapse:collapse; margin:16px 0;">
                    <tr>
                        <td style="padding:6px 0; color:#6b7280;">Date</td>
                        <td style="padding:6px 0; font-weight:bold;">{{ $connecteLe }}</td>
                    </tr>
                    <tr>
                        <td style="padding:6px 0; color:#6b7280;">Adresse IP</td>
                        <td style="padding:6px 0; font-weight:bold;">{{ $adresseIp }}</td>
                    </tr>
                    <tr>
                        <td style="padding:6px 0; color:#6b7280;">Compte</td>
                        <td style="padding:6px 0; font-weight:bold;">{{ $user->email }}</td>
                    </tr>
                </table>
                <p style="color:#6b7280; font-size:13px;">
                    Si vous n'êtes pas à l'origine de cette connexion, changez votre mot de passe dès que possible.
                </p>
            </td>
        </tr>
    </table>
</body>
</html>
